...
import { STATUS_TIER } from '../data/statusColors';
...
function applyAll(records: LniRecord[], state: GridState, getCustomValue?: (id: string, key: string) => unknown): LniRecord[] {
  ...
  if (state.manualOrder) {
    ...
    // Always sort by status tier first so the workflow order is preserved.
    // User-chosen sorts are applied as secondary criteria within each tier.
    result = [...result].sort((a, b) => {
      const ta = STATUS_TIER[a.status] ?? 8;
      const tb = STATUS_TIER[b.status] ?? 8;
      if (ta !== tb) return ta - tb;

      // Within the same tier, apply user sorts
