...
export const STATUS_COLORS: Record<string, BadgeColor> = {
  ...
  'Billboards':                 'gray',
  'Surveys':                    'gray',
  'MFG - Len Metal Fab':        'orange',
  'MFG - Terry Metal Fab':      'orange',
  'MFG - Chris Metal Fab':      'orange',
};
...
export const SALES_COLORS: Record<string, BadgeColor> = {
  'LNI': 'navy',
  'CC':  'blue',
  'NH':  'purple',
  'DW':  'green',
...
// Primary sort tier — records within the same tier are sorted by user-chosen criteria.
// Lower number = higher in the list.
export const STATUS_TIER: Record<string, number> = {
  // 1 — Completed / Admin
  'Complete to Admin':          1,
  'Complete Invoiced':          1,
  'End of Active Work':         1,

  // 2 — Installation
  'Installation':               2,
  'Needs Shipped':              2,

  // 3 — Active Manufacturing
  'Manufacturing':              3,
  'Active':                     3,
  'MFG - Assembly':             3,
  'MFG - Routing':              3,
  'MFG - Vinyl Application':    3,
  'MFG - Paint Prep / Paint':   3,
  'MFG - Assembly & Graphics':  3,
  'MFG - Need Material Cut':    3,
  'MFG - Vinyl Cut':            3,
  'MFG - Vinyl Install':        3,
  'Steel MFG':                  3,
  'Upcoming Mfg.':              3,
  'MFG - Len Metal Fab':        3,
  'MFG - Terry Metal Fab':      3,
  'MFG - Chris Metal Fab':      3,

  // 4 — Subcontracted / External
  'Subcontracted':              4,
  'Outsourced - Vendor':        4,
  'NEK - Production':           4,
  'Service or Contract Order':  4,

  // 5 — Hold
  'Hold - Customer':            5,
  'Hold - Permit':              5,
  'Hold - Local':               5,
  'Hold - Product Ready':       5,

  // 6 — New / Upcoming
  'New Order this week':        6,

  // 7 — Miscellaneous
  'Refurb - Awaiting Removal':  7,
  'Billboards':                 7,
  'Surveys':                    7,
};
...
export function getBadgeColor(field: string, value: string): BadgeColor {
  ...
  if (field === 'sales')        return SALES_COLORS[value] ?? 'gray';
  if (['region','mfgRegion','installRegion'].includes(field)) return REGION_COLORS[value] ?? 'gray';
  if (['metal','assembly','plex','paintPrep','materialCut','vinylProd'].includes(field)) return STAGE_COLORS[value] ?? 'gray';
  return 'gray';
}
