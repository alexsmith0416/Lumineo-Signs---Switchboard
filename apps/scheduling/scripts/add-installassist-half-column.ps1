<#
  Adds one column to crfdf_installationemployees so an "assist installation"
  day can be a HALF day (AM or PM) instead of the whole day:

    crfdf_assisthalves (Text) - per-partial-day half, "day:half" comma list
                                (e.g. "1:am,3:pm"). Days not listed = full day.
                                Blank on existing rows = every assist day is full.

  Complements crfdf_assistdays (which days) / crfdf_assistweekstart (which week).
...
Write-Host "Adding assist-half column to $entity ..." -ForegroundColor Cyan
Str 'crfdf_AssistHalves' 'Assist Halves' 60
