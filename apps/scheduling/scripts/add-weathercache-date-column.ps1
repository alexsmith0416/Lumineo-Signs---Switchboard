<#
  Adds a DateOnly column `lum_date` to the lum_weathercache Dataverse table so the
  weather cache can hold a per-day FORECAST (one row per ZIP per date) instead of a
  single current-conditions row per ZIP. Uses the Web API with a device-code token
  (az is blocked by the proxy; this uses raw Invoke-RestMethod).
...
function New-DateColumn($schema, $label) {
...
Write-Host "Adding forecast-date column to $entity ..." -ForegroundColor Cyan
New-DateColumn 'lum_Date' 'Forecast Date'
