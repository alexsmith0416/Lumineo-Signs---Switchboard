<#
  Adds the split-group column to the schedule-line + install-card tables:

    crfdf_productionscheduleline.crfdf_SplitGroup  (text, 100)
    crfdf_installcard.crfdf_SplitGroup             (text, 100)

  Links the parts of a job task scheduled in SECTIONS (right-click a card ->
  "Split into sections..."). Every part carries the same group id, the same
  unchanged crfdf_estimatedhours pot, and its own slice of it in
  crfdf_overridehours. Blank = a normal, whole card.

  The app works without this column — it just falls back to grouping parts by
  same job + task + employee, so a part moved to a DIFFERENT person would stop
  counting against the same pot. Run this to make the link explicit.
...
function New-TextColumn($entity, $schema, $label, $maxLen) {
...
Write-Host "Adding crfdf_SplitGroup…" -ForegroundColor Cyan
foreach ($set in @('crfdf_productionschedulelines', 'crfdf_installcards')) {
  $entity = Resolve-Entity $set
  New-TextColumn $entity 'crfdf_SplitGroup' 'Split Group' 100
}
