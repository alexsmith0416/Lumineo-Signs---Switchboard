<#
  Creates the per-job department OVERRIDE table that backs the editable job
  stepper:

    crfdf_jobdeptoverride — one row per (job, department key) when an editor has
      overridden the BC-derived stepper for that job:
        crfdf_included (Yes/No, default Yes) — Yes = force this department into
          the stepper (an "added" dept not in the BC planning lines); No = force
          it OUT (a "removed" dept that BC did include).
        crfdf_active (Yes/No, default No) — editor marked this department as an
          ADDITIONAL active step (on top of the default first-incomplete active).

  The base stepper is still derived from BC planning lines (jobStepInfo); these
  rows only layer editor edits on top. A row exists only when there's an override
  for that (job, dept). Completions stay in crfdf_jobdeptcompletion.
...
Write-Host "Job department override table…" -ForegroundColor Cyan
New-Table 'crfdf_JobDeptOverride' 'Job Dept Override' 'Job Dept Overrides'
New-Column     'crfdf_jobdeptoverride' 'crfdf_JobNo'    'Job No'      40
New-Column     'crfdf_jobdeptoverride' 'crfdf_DeptId'   'Dept Id'     10
New-BoolColumn 'crfdf_jobdeptoverride' 'crfdf_Included' 'Included'    $true
New-BoolColumn 'crfdf_jobdeptoverride' 'crfdf_Active'   'Active'      $false
