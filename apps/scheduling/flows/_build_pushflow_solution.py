"""Assemble an importable UNMANAGED solution (BCPushReview v1.0.0.1) containing the
BCPush_PlanningSteps flow — the app->BC write-back flow that drains the
crfdf_bcpushqueue outbox and PATCHes the sign365 projectPlanningEntries API.

Self-contained (unlike _build_solution.py it doesn't reuse an export): it authors
the <Workflow> metadata + a single Dataverse <connectionreference> inline.
...
On import (maker portal → Solutions → Import solution):
  ...
  - then open the flow, fill the Bc_* parameters (secret via env var / Key Vault),
    and turn it On.
"""
...
OUT_ZIP = r"C:\Users\Alex\Downloads\BCPushReview_1_0_0_1.zip"

FLOW_NAME    = "BCPush_PlanningSteps"
FLOW_GUID    = "0c5c184d-9e8c-4b0c-a781-56954c629083"
SRC_JSON     = "BCPush_PlanningSteps-clientdata.json"
DEST_JSON    = f"{FLOW_NAME}-{FLOW_GUID.upper()}.json"
CONNREF_NAME = "new_sharedcommondataserviceforapps_bcpush"

workflow_block = f'''<Workflow WorkflowId="{{{FLOW_GUID}}}" Name="{FLOW_NAME}" Description="Drains crfdf_bcpushqueue and PATCHes the sign365 projectPlanningEntries API. Built by agent.">
      ...
      <Descriptions>
        <Description languagecode="1033" description="Drains crfdf_bcpushqueue and PATCHes the sign365 projectPlanningEntries API. Built by agent." />
      </Descriptions>
    </Workflow>'''
